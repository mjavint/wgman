...
func Favicon() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		return c.Redirect(http.StatusFound, util.BasePath+"/favicon.ico")
	}
}
...
func Logout() echo.HandlerFunc {
	return func(c echo.Context) error {
		clearSession(c)
		return c.JSON(http.StatusOK, jsonHTTPResponse{true, "Logged out successfully"})
	}
}
...
func ApplyServerConfig(db store.IStore, tmplDir fs.FS) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if util.ManageStart || util.ManageRestart {
			log.Infof("Applying config and restarting WireGuard interface...")
			err = util.RestartWireGuard(settings.ConfigFilePath)
			if err != nil {
				log.Warnf("Cannot restart WireGuard: %v (this requires running inside a container with NET_ADMIN)", err)
			}
		}

		return c.JSON(http.StatusOK, APIResponse{Success: true, Message: "Applied server config successfully"})
	}
}
...
// GetPendingChanges returns if there are pending changes to apply
func GetPendingChanges(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		hasChanges := util.HashesChanged(db)
		return c.JSON(http.StatusOK, APIResponse{
			Success: true,
			Data:    map[string]interface{}{"has_changes": hasChanges},
		})
	}
}
