...
func ValidSession(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		if !isValidSession(c) {
			// For API routes return 401 JSON so the React frontend can handle it
			if strings.HasPrefix(c.Request().URL.Path, util.BasePath+"/api/") ||
				c.Request().Method != http.MethodGet {
				return c.JSON(http.StatusUnauthorized, jsonHTTPResponse{false, "Unauthorized"})
			}
