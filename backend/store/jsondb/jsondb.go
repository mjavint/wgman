...
func (o *JsonDB) Init() error {
	...
	if _, err := os.Stat(globalSettingPath); os.IsNotExist(err) {
		...
		if endpointAddress == "" {
			log.Warn("No endpoint address configured and external IP lookup disabled")
...
func (o *JsonDB) GetClients(hasQRCode bool) ([]model.ClientData, error) {
	...
	if err != nil {
		log.Warnf("Cannot get server: %v", err)
		server = model.Server{}
	}
	...
	for _, f := range records {
		...
		if hasQRCode && client.PrivateKey != "" && client.AllocatedIPs != nil {
			if server.KeyPair == nil || server.Interface == nil {
				log.Warnf("Server not configured for QR code")
				continue
			}

			if globalSettings.DNSServers == nil || len(globalSettings.DNSServers) == 0 {
				globalSettings.DNSServers = []string{"8.8.8.8", "8.8.4.4"}
			}

			mtuVal := globalSettings.MTU
			if mtuVal <= 0 {
				mtuVal = 1420
			}
			globalSettings.MTU = mtuVal

			keepaliveVal := globalSettings.PersistentKeepalive
			if keepaliveVal <= 0 {
				keepaliveVal = 25
			}
			globalSettings.PersistentKeepalive = keepaliveVal

			if globalSettings.EndpointAddress == "" {
				globalSettings.EndpointAddress = "192.168.16.62"
			}

			log.Debugf("QR config: Endpoint=%s:%d, DNS=%v, MTU=%d, Keepalive=%d, UseDNS=%v",
				globalSettings.EndpointAddress,
				server.Interface.ListenPort,
				globalSettings.DNSServers,
				globalSettings.MTU,
				globalSettings.PersistentKeepalive,
				client.UseServerDNS)

			config := util.BuildClientConfig(client, server, globalSettings)
			log.Debugf("Generating QR for client %s config:\n%s", client.ID, config)

			png, err := qrcode.Encode(config, qrcode.Medium, 256)
			if err == nil {
				clientData.QRCode = base64.StdEncoding.EncodeToString(png)
				log.Infof("Generated QR code for client %s", client.Name)
			} else {
				log.Errorf("Cannot generate QR code for %s: %v", client.Name, err)
			}
		}
