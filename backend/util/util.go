...
func GetPublicIP() (model.Interface, error) {
	...
	if err != nil {
		log.Warn("Cannot get public IP address: ", err)
...
func HashesChanged(db store.IStore) bool {
	old, err := db.GetHashes()
	if err != nil {
		// No saved hashes yet — treat as changed so Apply button appears
		return true
	}
...
var ManageStart bool
var ManageRestart bool

func InitManageFlags() {
	ManageStart = LookupEnvOrBool(ManageStartEnvVar, false)
	ManageRestart = LookupEnvOrBool(ManageRestartEnvVar, false)
	if ManageStart || ManageRestart {
		log.Infof("WireGuard management enabled: ManageStart=%v, ManageRestart=%v", ManageStart, ManageRestart)
	}
}
...
func IsWireGuardRunning(configPath string) bool {
	ifName := GetInterfaceName(configPath)
	cmd := exec.Command("sudo", "-n", "wg", "show", ifName)
	return cmd.Run() == nil
}

func StartWireGuard(configPath string) error {
	if _, err := os.Stat(configPath); err != nil {
		return fmt.Errorf("config file not found: %v", err)
	}

	cmd := exec.Command("sudo", "-n", "wg-quick", "up", configPath)
	output, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("failed to start WireGuard: %v, output: %s", err, string(output))
	}
	log.Infof("WireGuard started successfully")
	return nil
}

func StopWireGuard(configPath string) error {
	if _, err := os.Stat(configPath); err != nil {
		return fmt.Errorf("config file not found: %v", err)
	}

	cmd := exec.Command("sudo", "-n", "wg-quick", "down", configPath)
	output, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("failed to stop WireGuard: %v, output: %s", err, string(output))
	}
	log.Infof("WireGuard stopped successfully")
	return nil
}

func RestartWireGuard(configPath string) error {
	if err := StopWireGuard(configPath); err != nil {
		log.Warnf("Error stopping WireGuard: %v", err)
	}
	time.Sleep(500 * time.Millisecond)
	if err := StartWireGuard(configPath); err != nil {
		return err
	}
	log.Infof("WireGuard restarted successfully")
	return nil
}
